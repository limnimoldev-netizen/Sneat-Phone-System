...
<div class="container-xxl flex-grow-1 container-p-y">
    <h4 class="fw-bold py-3 mb-4">{{__('category.color.colors_management')}}</h4>
    <div class="row">
    ...
    <!-- List Role Table -->
    <div class="card">
        <h5 class="card-header">{{__('category.color.list')}}</h5>
        <div class="table-responsive text-nowrap">
            <table class="table mb-5">
                <thead>
                    <tr>
                        <th class="col-md-5">Product Colors</th>
                ...
                <tbody class="table-border-bottom-0">
                    ...
                    <tr>
                        ...
                        <td>{{ $color->products_status_sold_count }}</td>
                        <td>{{ $color->products_status_loan_count }}</td>
                        <td>{{ $color->products_status_instock_count }}</td>
                        <td>
            ...
            <div class="mt-3">
                <div class="modal fade" id="basicModal" tabindex="-1" aria-hidden="true">
                    <div class="modal-dialog" role="document">
                        <form action="{{ route('color.store', withLang())}}" method="post">
                            @csrf
                            <div class="modal-content">
                                <div class="modal-header">
                                    ...
                                    <div class="modal-body">
                                        <div class="row">
                                            <div class="col mb-3">
                                                <label for="name" class="form-label">{{__('category.color.product_colors')}}</label>
        ...
        <div class="col-lg-4 col-md-6">
            <div class="mt-3">
                <div class="modal fade" id="editModelType" tabindex="-1" aria-hidden="true">
                    <div class="modal-dialog" role="document">
                        <form action="{{ route('color.update', withLang())}}" method="post">
                            @csrf
                            <div class="modal-content">
                                <div class="modal-header">
                                <h5 class="modal-title" id="exampleModalLabel1">{{__('category.color.edit')}}</h5>
                                    <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                                </div>
                                <div class="modal-body">
                                    <div class="row">
                                    ...
                                    <div class="row">
                                        <div class="col mb-3">
                                            <label for="brand-name" class="form-label">{{__('category.add_new')}}</label>
                                            <input type="text" id="brand-name" name="name" class="form-control  @error('name') is-invalid @enderror" placeholder="Ex: Violet, Pink,">
                                        </div>
                                        <div id="myDiv" style="display: none" class="text-danger">{{__('category.color.exists')}}</div>
                                    </div>
                                </div>
                                <div class="modal-footer">
                                    <button type="button" class="btn btn-outline-secondary" data-bs-dismiss="modal">{{__('button.close')}}</button>
